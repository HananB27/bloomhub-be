...
from core.enums import LeaveRequestStatus, LeaveType, ProjectAssignmentStatus
...
def _setup_leave(employee_profile, *, leave_type=LeaveType.VACATION, allocated=10, used=0):
...
def test_calculate_and_validate_leave_request_branches():
    ...
    assert leave_service.calculate_working_days(today + timedelta(days=2), today + timedelta(days=1)) == 0

    ok, msg = leave_service.validate_leave_request(profile, LeaveType.VACATION, today - timedelta(days=1), today)
    assert ok is False and "past" in msg.lower()

    ok, msg = leave_service.validate_leave_request(profile, LeaveType.VACATION, today + timedelta(days=1), today + timedelta(days=3))
    assert ok is False and "notice" in msg.lower()

    ok, msg = leave_service.validate_leave_request(profile, "missing", today + timedelta(days=3), today + timedelta(days=4))
    assert ok is False and "not found" in msg.lower()

    covering, _ = _create_user("cover")
    LeavePolicy.objects.filter(leave_type=LeaveType.VACATION).update(requires_covering_employee=True)
    ok, msg = leave_service.validate_leave_request(profile, LeaveType.VACATION, today + timedelta(days=5), today + timedelta(days=8))
    ...
    LeavePolicy.objects.filter(leave_type=LeaveType.VACATION).update(requires_covering_employee=False, min_notice_in_days=0)
    LeaveBalance.objects.filter(employee=profile, leave_type=LeaveType.VACATION).update(allocated=1, used=1)
    ok, msg = leave_service.validate_leave_request(
        profile,
        LeaveType.VACATION,
    ...
    LeaveBalance.objects.filter(employee=profile, leave_type=LeaveType.VACATION).update(allocated=10, used=0)
...
def test_leave_approval_rejection_and_balance_changes(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr("core.services.mail.leave_notifications.notify_approver_confirmation", lambda *a, **k: calls.append(("conf", a, k)))
    monkeypatch.setattr("core.services.mail.leave_notifications.notify_employee_lead_decision", lambda *a, **k: calls.append(("lead", a, k)))
    monkeypatch.setattr("core.services.mail.leave_notifications.notify_hr_lead_approved", lambda *a, **k: calls.append(("hr", a, k)))
    monkeypatch.setattr("core.services.mail.leave_notifications.notify_employee_hr_decision", lambda *a, **k: calls.append(("hr_dec", a, k)))
    ...
    balance = LeaveBalance.objects.get(employee=emp_profile, leave_type=LeaveType.VACATION, year=timezone.now().year)
...
def test_leave_adjustment_carryover_and_team_members(monkeypatch):
    ...
    project = Project.objects.create(name="Shared", project_type="internal", status="active")
    ...
    monkeypatch.setattr(leave_service, "_has_permission", lambda user, module, actions: actions == ["adjust_balances"])
    assert leave_service.get_vacation_capabilities(SimpleNamespace(is_staff=False, is_superuser=False)) == {
    ...
    LeavePolicy.objects.filter(leave_type=LeaveType.VACATION).update(carryover_days=3, allocated_days_per_year=10)
    LeaveBalance.objects.filter(employee=emp_profile, leave_type=LeaveType.VACATION, year=timezone.now().year).update(used=4)
    ok, msg = leave_service.apply_carryover(emp_profile, LeaveType.VACATION, timezone.now().year, timezone.now().year + 1)
    assert ok is True and msg is None
    next_balance = LeaveBalance.objects.get(employee=emp_profile, leave_type=LeaveType.VACATION, year=timezone.now().year + 1)
    assert next_balance.carryover == 3

    leave_service.initialize_leave_balances_for_employee(other_profile, year=timezone.now().year + 2)
    assert LeaveBalance.objects.filter(employee=other_profile, year=timezone.now().year + 2).exists()
